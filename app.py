...
# ============================================================================
# FEATURES INCLUDED:
# 1. Data integration (Yahoo Finance, Local Cache)
...
class AdvancedPortfolioOptimizer:
    ...
    """
    def __init__(self, returns: pd.DataFrame, risk_free_rate: float = 0.05, frequency: int = 252):
        # NOTE:
        # - This optimizer expects **returns**, not prices.
        # - We compute expected returns and covariance directly from returns to avoid double-differencing.
        self.frequency = int(frequency) if frequency else 252
        self.risk_free_rate = float(risk_free_rate)

        # Sanitize returns for numerical stability (critical for Ledoit-Wolf / sklearn)
        self.returns = self._sanitize_returns(returns)

        self.n_assets = self.returns.shape[1]
        self.tickers = self.returns.columns.tolist()

        # Expected returns (annualized) & covariance (annualized) from returns directly
        self.mu = (self.returns.mean() * self.frequency).astype(float)
        self.S = self.returns.cov() * self.frequency

        # Shrinkage covariance estimators (return-based)
        self.S_ledoit_wolf = self._cov_ledoit_wolf(self.returns)
        self.S_oracle_approx = self._cov_oas(self.returns)

        logger.info(f"PortfolioOptimizer initialized with {self.n_assets} assets | freq={self.frequency}")

    @staticmethod
    def _sanitize_returns(df: pd.DataFrame) -> pd.DataFrame:
        """Ensure returns are numeric, finite, aligned, and NaN-free."""
        ...
        if x.shape[1] < 1:
            raise ValueError("No valid return series after cleaning (all-NA).")

        # truth-first: only keep dates where all assets have returns
        x = x.dropna(axis=0, how="any")

        if x.shape[0] < 30:
            raise ValueError(
                f"Insufficient clean return observations after NaN removal: {x.shape[0]} rows. "
                "Try a longer date range or fewer assets."
            )

        return x

    def _cov_ledoit_wolf(self, returns_df: pd.DataFrame) -> pd.DataFrame:
        """Ledoit-Wolf shrinkage covariance from returns (annualized)."""
        if not HAS_SKLEARN:
            logger.info("scikit-learn not available; using sample covariance instead of Ledoit-Wolf.")
            return self.S

        try:
            lw = LedoitWolf().fit(returns_df.values)
            cov = pd.DataFrame(lw.covariance_, index=returns_df.columns, columns=returns_df.columns)
            return cov * self.frequency
        except Exception as e:
            logger.warning(f"Ledoit-Wolf covariance failed; falling back to sample covariance. Reason: {e}")
            return self.S

    def _cov_oas(self, returns_df: pd.DataFrame) -> pd.DataFrame:
        """Oracle Approximating Shrinkage covariance from returns (annualized)."""
        if not HAS_SKLEARN:
            logger.info("scikit-learn not available; using sample covariance instead of OAS.")
            return self.S

        try:
            oas = OAS().fit(returns_df.values)
            cov = pd.DataFrame(oas.covariance_, index=returns_df.columns, columns=returns_df.columns)
            return cov * self.frequency
        except Exception as e:
            logger.warning(f"OAS covariance failed; falling back to sample covariance. Reason: {e}")
            return self.S
